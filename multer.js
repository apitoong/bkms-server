...
app.post("/", function(req, res, next) {
  ...
  var prog = progress({ time: 100 }, function(progress) {
    // time:100 means will check progress every 100 ms, say to update progress bar
  ...
  upload(prog, res, function(err) {
    // changed req to prog in order to track % upload progress
    if (err) {
      res
        .status(err.status || 500)
        .json({ error: { status_code: err.status || 500, message: err.code } });
      return;
    } else {
      ...
      if (prog.files.fileName) {
        // fileName comes from input element:   <input type="file" name="fileName">
        ...
        if (urlParsed.pathname) {
          var onlyTheFilename = urlParsed.pathname
            ? urlParsed.pathname
          ...
          var newFilename =
            onlyTheFilename + "-" + Date.now() + "-" + onlyTheFilename;
          ...
          download(urlParsed.href, newFilename, function() {
            ...
            res.end(
              "<h1>Uploaded from URL</h2><img style='max-width:50%' src='" +
                wholePath +
                "'/><pre>" +
                reqJSON +
